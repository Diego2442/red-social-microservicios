...
import { formatDate, formatDateBirth } from "@/src/utils";

export default function Profile({user}: {user: User}) {
  return (
    <section className="px-2 py-4 mt-16 flex flex-col justify-center items-center text-center">
        <img
          ...
          alt=""
        />
        <h1 className="text-2xl text-gray-500 font-bold mt-2">{user.user_name}</h1>
        <h2 className="text-base md:text-xl text-gray-500 font-bold">
          {user.full_name}
        </h2>
        <span className="text-sm font-light text-gray-600 dark:text-gray-400">
                  {user.birth_date&&formatDateBirth(user.birth_date)}
                </span>
